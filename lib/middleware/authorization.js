 ...
 * @api protected
 */
module.exports = function a(server, options, validate, immediate, complete) {
  ...
  options = options || {};
  immediate = immediate || function i(client, user, done) { return done(null, false); };
  ...
  return function authorization(req, res, next) {
    ...
    server._parse(type, req, (err, areq) => {
      ...
      function validated(err2, client, redirectURI, webOrigin) {
        ...
        function immediated(err3, allow, info, locals) {
          ...
          if (allow) {
            ...
            return server._respond(req.oauth2, res, completing, (err4) => {
          ...
          if (server._txnStore.legacy === true) {
            ...
            return server._txnStore.store(server, options, req, txn, stored);
          }
          return server._txnStore.store(req, req.oauth2, stored);
        }
      ...
      try {
        ...
        if (arity === 5) {
          return validate(areq.clientID, areq.redirectURI, areq.scope, areq.type, validated);
        }
        if (arity === 4) {
          return validate(areq.clientID, areq.redirectURI, areq.scope, validated);
        }
        if (arity === 3) {
          return validate(areq.clientID, areq.redirectURI, validated);
        }
        // arity == 2
        return validate(areq, validated);
      } catch (ex) {
        return next(ex);
      }
    });
