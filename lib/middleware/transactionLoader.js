 ...
 *     sessionKey        key under which transactions are stored in the
 *                       session (default: 'authorize')
 *
 * @param {Server} server
...
module.exports = function loader(server, options) {
  ...
  return function transactionLoader(req, _, next) {
    ...
    if (server._txnStore.legacy === true) {
      return server._txnStore.load(server, options, req, loaded);
    }
    return server._txnStore.load(req, loaded);
  };
};
