 ...
 * Examples:
 *
 *     server.exchange(oauth2orize.exchange.authorizationCode(function(client, code, redirectURI,
 *      done) {
 ...
 * @api public
 */
module.exports = function a(options, issue) {
  ...
  return function authorization_code(req, res, next) {
    ...
    try {
      ...
      if (arity === 6) {
        return issue(client, code, redirectURI, req.body, req.authInfo, issued);
      }
      if (arity === 5) {
        return issue(client, code, redirectURI, req.body, issued);
      }
      // arity == 4
      return issue(client, code, redirectURI, issued);
    } catch (ex) {
      return next(ex);
    }
  };
