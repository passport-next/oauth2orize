 ...
 * @api public
 */
module.exports = function c(options, issue) {
  ...
  return function client_credentials(req, res, next) {
    ...
    try {
      ...
      if (arity === 5) {
        return issue(client, scope, req.body, req.authInfo, issued);
      }
      if (arity === 4) {
        return issue(client, scope, req.body, issued);
      }
      if (arity === 3) {
        return issue(client, scope, issued);
      }
      // arity == 2
      return issue(client, issued);
    } catch (ex) {
      return next(ex);
    }
  };
