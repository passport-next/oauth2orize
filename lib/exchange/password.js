 ...
 * @api public
 */
module.exports = function p(options, issue) {
  ...
  return function password(req, res, next) {
    ...
    try {
      ...
      if (arity === 7) {
        return issue(client, username, passwd, scope, req.body, req.authInfo, issued);
      }
      if (arity === 6) {
        return issue(client, username, passwd, scope, req.body, issued);
      }
      if (arity === 5) {
        return issue(client, username, passwd, scope, issued);
      }
      // arity == 4
      return issue(client, username, passwd, issued);
    } catch (ex) {
      return next(ex);
    }
  };
