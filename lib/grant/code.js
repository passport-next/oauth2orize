...
module.exports = function code(options, issue) {
  ...
  function response(txn, res, complete, next) {
    ...
    function issued(err, code2) {
      if (err) { return next(err); }
      if (!code2) { return next(new AuthorizationError('Request denied by authorization server', 'access_denied')); }

      const params = { code: code2 };
      if (txn.req && txn.req.state) { params.state = txn.req.state; }
    ...
    try {
      ...
      if (arity === 7) {
        return issue(txn.client, txn.req.redirectURI, txn.user, txn.res, txn.req,
          txn.locals, issued);
      }
      if (arity === 6) {
        return issue(txn.client, txn.req.redirectURI, txn.user, txn.res, txn.req, issued);
      }
      if (arity === 5) {
        return issue(txn.client, txn.req.redirectURI, txn.user, txn.res, issued);
      } // arity == 4
      return issue(txn.client, txn.req.redirectURI, txn.user, issued);
    } catch (ex) {
      return next(ex);
    }
  }
