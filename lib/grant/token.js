...
const Fragment = require('../response/fragment');
...
module.exports = function token(options, issue) {
  ...
  function response(txn, res, complete, next) {
    ...
    try {
      ...
      if (arity === 6) {
        return issue(txn.client, txn.user, txn.res, txn.req, txn.locals, issued);
      }
      if (arity === 5) {
        return issue(txn.client, txn.user, txn.res, txn.req, issued);
      }
      if (arity === 4) {
        return issue(txn.client, txn.user, txn.res, issued);
      } // arity == 3
      return issue(txn.client, txn.user, issued);
    } catch (ex) {
      return next(ex);
    }
  }
