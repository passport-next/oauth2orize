...
const authorizationCode = require('../../lib/exchange/authorizationCode');


describe('exchange.authorizationCode', () => {
  it('should be named authorization_code', () => {
    expect(authorizationCode(() => {}).name).to.equal('authorization_code');
  });
  ...
  describe('issuing an access token based on body', () => {
    let response; let
      err;
  ...
  describe('handling a request without code parameter', () => {
    let response; let
      err;
